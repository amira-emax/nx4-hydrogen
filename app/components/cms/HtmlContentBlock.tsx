    ...
    replace: (domNode) => {
      if (domNode instanceof Element) {
        ...
        if (domNode.name === 'img') {
          ...
          // Check if next element is h6 (caption)
          let nextNode = (domNode as any).next;
          while (
            nextNode &&
            nextNode.type === 'text' &&
            !nextNode.data?.trim()
          ...
          const captionNode =
            nextNode && nextNode.name === 'h6' ? nextNode : null;

          if (captionNode) {
            // Prevent the h6 from being rendered again
            captionNode.type = 'text';
            captionNode.data = '';

            return (
              <div className="my-[40px]">
                <img {...props} className="w-full h-auto mb-1" />
                <h6 className="text-body-medium text-secondary">
                  {domToReact(captionNode.children as DOMNode[], {
                    ...getBlogOptions(),
                    replace: (childNode) => {
                      if (
          ...
          return (
            <img {...props} className="w-full h-auto mb-[24px] md:mb-[48px]" />
          );
        }
        ...
        if (domNode.name === 'h6') {
          ...
          return (
            <h6
              {...props}
              className="text-body-medium text-secondary mb-[12px] md:mb-[24px]"
            >
              {domToReact(domNode.children as DOMNode[], getBlogOptions())}
            </h6>
          );
        ...
        if (domNode.name === 'p') {
          ...
          // Regular paragraph
          return (
            <p className="text-h3-regular mb-[12px] md:mb-[24px]" {...props}>
              {domToReact(domNode.children as DOMNode[], getBlogOptions())}
            </p>
          );
        }
    ...
    replace: (domNode) => {
      if (domNode instanceof Element) {
        ...
        if (domNode.name === 'h2' || domNode.name === 'h3') {
          return (
            <h2 className="mb-[20px] mt-[40px] md:mt-[56px]">
              {domToReact(domNode.children as DOMNode[], getPolicyOptions())}
            </h2>
          );
        }
        ...
        if (domNode.name === 'p') {
          ...
          return (
            <p className="text-h3-regular mb-[12px] md:mb-[18px]" {...props}>
              {domToReact(domNode.children as DOMNode[], getPolicyOptions())}
            </p>
          );
        }
        ...
        if (domNode.name === 'ul') {
          return (
            <ul className="text-h3-regular list-disc pl-6 mb-[24px]">
              {domToReact(domNode.children as DOMNode[], getPolicyOptions())}
            </ul>
          );
        }
